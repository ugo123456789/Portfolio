...
const NavBar = () => {
  ...
  return (
    <nav className="flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed">
      <div>
        <h1 className="text-5xl font-signature ml-2">Ugonna</h1>
      </div>
      ...
      {nav && (
        ...
        bg-gradient-to-b from-black to-gray-800 text-gray-400 space-y-6'>
         {links.map((link) => {
            return (
              <li
                key={link.id}
                className="px-4 cursor-pointer capitalize font-medium text-gray-400 hover:scale-110 duration-200"
              >
              <Link  onClick={() => setNav(!nav)  }  to={link.link} smooth duration={700}>{link.link}</Link>
              </li>
            );
          })}
